...
using namespace std;
namespace seneca {
	//trin leading and trailing space
	void trim(string& fName){
		if (fName.length() > 0) {
			int begin = fName.find_first_not_of(' ');
			int end = fName.find_last_not_of(' ');
			if (begin == -1) fName = "";
			else fName = fName.substr(begin, end - begin + 1);
		}
	}
	...
	std::list<Song> SongCollection::getSongsForArtist(const std::string& artistName) const
	{
		...
		auto it = copy_if(m_songs.begin(), m_songs.end(), songlist.begin(), [&artistName](const Song& song) {return song.m_artist == artistName; });
